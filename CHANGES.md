- 20240202 version: notqmail 1.09.
...
- 20221116 process: CI: update Solaris and add OpenBSD runner.
- 20221116 code: add additional groups for `qmail-remote`.
- 20221023 code: netstring parsing in `qmail-qmtpd` & `qmail-qmqpd` now
           checks that the netstring length is actually a number.
- 20221016 process: CI: update GitHub Actions to Ubuntu 22.04.
- 20221013 code: add missing system headers for `close()`, `chdir()`, `getpid()`.
- 20220510 code: `qmail-qstat`: simplify the code.
- 20220504 process: update CirrusCI FreeBSD autobuild.
- 20220215 bug: `qmail-inject`: fix header parse regression.
- 20220130 code: Add tests for `qmail-send:job_*()` functions.
- 20220119 process: Fix Solaris autobuilds; update Ubuntu autobuild to a recent libcheck.
- 20210816 code: `Makefile`: remove stale references to `auto-uid`.
- 20210815 process: fix `make clean`.
- 20210809 process: De-symlink `.gitignore` to mollify new Git.
- 20210701 process: CI: update FreeBSD, remove TravisCI.
- 20210130 code: add `noreturn` attribute to a couple of functions.
- 20210124 process: remove TravisCI.
- 20210124 process: CI: add Solaris.
...
- 20201216 process: Define `CODEOWNERS` with our default reviewers.
- 20201216 code: Fix CodeQL cpp/integer-multiplication-cast-to-long.
- 20201204 code: use `ssize_t` in `substdio`.
- 20201130 process: symlink autobuild configs somewhere easier to see.
...
- 20201108 process: extend usage of GitHub actions for CI builds.
...
- 20200901 process: GitHub: enable codescan.
...
- 20200421 process: update CirrusCI FreeBSD config.
